...
export default async function Page(props: Props) {
  ...
  const { items: relatedPost, error: categoryError } = await directus.post.search({
  ...
  return (
    <Container>
      <div className="max-w-screen-lg mx-auto">
        ...
        <h1 className="font-bold md:text-5xl leading-normal my-6">{post.title}</h1>
        ...
        {post.images?.length ? <PostImages images={post.images} /> : <PostImage image={post.cover as ImageType} />}
        {/* start content */}
        {post.blocks && <BlocksView blocks={post.blocks} className="mt-4 leading-loose text-lg" />}
        {/* end content */}
        <ShareArticle />
        <LatestNews posts={relatedPost} className="mt-5" title={`Tulisan Terkait:`} />
