  ...
  return (
    <div className="flex flex-col md:flex-row gap-4">
      ...
      <div className="flex-[1] rounded-md p-4">
        <h2 className="uppercase text-primary-600 font-semibold mb-4 tracking-widest text-xs">{item.subtitle}</h2>
        <h2 className={`scroll-m-20 tracking-tight lg:text-4xl mb-4 ${customFont.className}`}>{item.title}</h2>
        <div className="markdown" dangerouslySetInnerHTML={{ __html: item.content as string }} />
      </div>
    </div>
  );
}
  ...
  return (
    <>
      {homepage.blocks?.map((block, index) => (
        <div key={index}>{block.collection == 'block_hero' && <HomepageHero item={block.item as BlockHero} />}</div>
      ))}
    </>
  );
}
