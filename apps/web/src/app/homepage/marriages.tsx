  ...
  return (
    <>
      {items.length > 0 ? (
          ...
          {items.map((data, index) => (
            <div className="marriage-el-children-container" key={index}>
              <div className="marriage-el-children">
                ...
                <div
                  className={`text-2xl md:text-5xl text-primary-400 text-center p-4 ${frank.className}`}
                >
                  &
                </div>
        ...
        <div className="p-2 my-2 border text-sm text-center rounded bg-gray-100">
...
function MarriagesContainer({
  children,
  title,
  number,
  className,
}: MarriagesContainerProps) {
  return (
    <div className={`py-2 pb-3 ${className}`}>
      <h4 className="uppercase tracking-widest text-base">
        <span className="p-1 px-2 mr-3 text-sm bg-gray-200 text-gray-500 rounded">
          {number}
        </span>
        {title}
      </h4>
